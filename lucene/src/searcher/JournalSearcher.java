...
public class JournalSearcher {
    ...
    public static IndexSearcher getSearcher(String path) {
        try {
            if (searcher == null) {
                searcher = new IndexSearcher(Common.getFSDirectory(path, IndexConst.JOURNAL_INDEX_PATH));
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        ...
        try {
            ...
            if (topDocs != null && topDocs.totalHits > 0) {
                ...
                for (int i = 0; i < topDocs.totalHits; i++) {
                    ...
                    list.add(Integer.parseInt(doc.get(IndexConst.JOURNAL_IDJOURNAL_FIELD)));
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
